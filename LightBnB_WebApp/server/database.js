...
const getAllProperties = function(options, limit = 10) {
  ...
  // only return properties belonging to owner
  if (options.owner_id) {
    queryParams.push(`%${options.owner_id}%`);
    queryString += `AND owner_id = $${queryParams.length} `;
  }

  // only return properties within price range
  ...
  // only return properties with a rating equal or higher than minimum_rating
